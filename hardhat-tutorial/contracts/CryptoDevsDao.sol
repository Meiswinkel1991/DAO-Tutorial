...
contract CryptoDevsDAO is Ownable {
    ...
    modifier nftHolderOnly() {
        if (cryptoDevsNFT.balanceOf(msg.sender) <= 0) {
            revert CryptoDevsDAO__NotDAOMember();
        }
        _;
    }
    ...
    modifier inactiveProposalOnly(uint256 proposalIndex) {
        if (proposals[proposalIndex].deadline > block.timestamp) {
            revert CryptoDevsDAO__DeadLineNotExceed();
        }
        ...
        activeProposalOnly(proposalIndex)
    {
        ...
        if (numVotes <= 0) {
            revert CryptoDevsDAO__AlreadyVoted();
        }
        ...
        inactiveProposalOnly(proposalIndex)
    {
        ...
        if (proposal.yayVotes > proposal.nayVotes) {
            ...
            if (address(this).balance < nftPrice) {
                revert CryptoDevsDAO__NotEnoughFunds();
            }
